...
if __name__ == "__main__":
    ...
    titles = [
    r"(c) $\rho_0 a(\widetilde{B+D_V})\ (s=0.9km) $",                  r"(d) $\rho_0 a(\widetilde{B+D_V})\ (s=2.4km) $", 
    r"(g) $\rho_0 a((B + D_V)({\widetilde{\cdot }}))\ (s=0.9km) $",    r"(h) $\rho_0 a((B + D_V)({\widetilde{\cdot }}))\ (s=2.4km) $", 
    r"(k) c - g [$kg\cdot m^{-2}s^{-2}$]",                             r"(l) d - h [$kg\cdot m^{-2}s^{-2}$]",  
    ]
    ...
    for convObj in convDict:
        ...
        for prof in range(len(convObj["xAxis"])):
            ...
            plt.savefig(f"./figureB1-2.png", dpi=300)
            plt.clf()
