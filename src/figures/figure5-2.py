    ...
    for tIdx in timeArange:
        ...
        for stdIdx, std in enumerate(coarseReslList):
            ...
            if (coarseReslList[stdIdx] != 0.0):
                buoy = 1e3*np.mean(np.roll(nc.Dataset(config.rebuildDynPath + f"gaussian-{std:.1f}/a-{tIdx:06d}.nc")["a"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax], axis=(1, 2))
                plt.plot(rho*buoy, zc/1e3, c=rbCmap((stdIdx+1)/(len(coarseReslList)+1)), linewidth=lw)
            else:
                buoy = 1e3*np.mean(np.roll(nc.Dataset(config.rebuildDynPath + f"uniform-1/a-{tIdx:06d}.nc")["a"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax], axis=(1, 2))
            ...
            if (coarseReslList[stdIdx] != 0.0):
                dm03 = 1e3*np.mean(np.roll(nc.Dataset(config.dynTermDirectAccePath + f"gaussian-{std:.1f}/a-{tIdx:06d}.nc")["dm04"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax], axis=(1, 2))
                plt.plot(rho*dm03, zc/1e3, c=rbCmap((stdIdx+1)/(len(coarseReslList)+1)), linewidth=lw, label=f"{realSpaceLength[stdIdx]}km")
            else:
                dm03 = 1e3*np.mean(np.roll(nc.Dataset(config.dynTermDirectAccePath + f"uniform-1/a-{tIdx:06d}.nc")["dm04"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax], axis=(1, 2))
            ...
            if (coarseReslList[stdIdx] != 0.0):
                res  = np.roll(nc.Dataset(config.dynTermDirectAccePath + f"gaussian-{std:.1f}/a-{tIdx:06d}.nc")["dm05"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
                res += np.roll(nc.Dataset(config.dynTermDirectAccePath + f"gaussian-{std:.1f}/a-{tIdx:06d}.nc")["dm06"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
                res += np.roll(nc.Dataset(config.dynTermDirectAccePath + f"gaussian-{std:.1f}/a-{tIdx:06d}.nc")["dm07"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
                res /= 3
                plt.plot(rho*1e3*np.mean(res, axis=(1, 2)), zc/1e3, c=rbCmap((stdIdx+1)/(len(coarseReslList)+1)), linewidth=lw)
            else:
                
                res =  np.roll(nc.Dataset(config.dynTermDirectAccePath + f"uniform-1/a-{tIdx:06d}.nc")["dm05"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
                res += np.roll(nc.Dataset(config.dynTermDirectAccePath + f"uniform-1/a-{tIdx:06d}.nc")["dm06"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
                res += np.roll(nc.Dataset(config.dynTermDirectAccePath + f"uniform-1/a-{tIdx:06d}.nc")["dm07"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
                res /= 3
        ...
        plt.savefig(f"./figure5-2.png", dpi=300)
        plt.clf()
