...
if __name__ == "__main__":
    ...
    titles = [
    r"(a) $B$ [$m\cdot s^{-2}$]",                  r"(b) $\widetilde{B}\ (s=0.9km) $ [$m\cdot s^{-2}$]",                  r"(c) $\widetilde{B}\ (s=2.4km) $ [$m\cdot s^{-2}$]",
    r"(e) $D_V$ [$m\cdot s^{-2}$]",                r"(f) $\widetilde{D_V}\ (s=0.9km) $ [$m\cdot s^{-2}$]",                r"(g) $\widetilde{D_V}\ (s=2.4km) $ [$m\cdot s^{-2}$]",
    r"(i) $D_H$ [$\times 10^{-4} m^{-1} s^{-2}$]", r"(j) $\widetilde{D_H}\ (s=0.9km) $ [$\times 10^{-4} m^{-1} s^{-2}$]", r"(k) $\widetilde{D_H}\ (s=2.4km) $ [$\times 10^{-4} m^{-1} s^{-2}$]",
    ]
    ...
    for convObj in convDict:
        ...
        for prof in range(len(convObj["xAxis"])):
            ...
            #cb.ax.tick_params(labelsize=30)
            #fig.tight_layout()
            plt.savefig(f"./figure4-1.png", dpi=300)
            plt.clf()
