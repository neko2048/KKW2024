...
if __name__ == "__main__":
    ...
    section = (24300)/1
    qc = np.roll(thData["qc"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
    qi = np.roll(thData["qi"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
    buoyOrigin = np.roll(nc.Dataset(config.rebuildDynPath + f"uniform-1/a-{tIdx:06d}.nc")["a"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
    dgData = nc.Dataset(f"{config.dynTermDirectAccePath}uniform-1/a-{tIdx:06d}.nc")
    dm03Origin = np.roll(dgData["dm04"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
    resOrigin = np.roll(dgData["dm05"][0][:, :, np.argmin(np.abs(xc-section))],  0, axis=-1)
    resOrigin += np.roll(dgData["dm06"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
    resOrigin += np.roll(dgData["dm07"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
    ...
    for i in range(len(coarseReslList)):
        ...
        if coarseReslList[i] != 0.0:
            coarseBuoy = np.roll(nc.Dataset(config.rebuildDynPath + f"gaussian-{coarseReslList[i]}/a-{tIdx:06d}.nc")["a"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
            dgData = nc.Dataset(f"{config.dynTermDirectAccePath}gaussian-{coarseReslList[i]}/a-{tIdx:06d}.nc")
            coarseDm03 = np.roll(dgData["dm04"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
            coarseRes =  np.roll(dgData["dm05"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
            coarseRes += np.roll(dgData["dm06"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
            coarseRes += np.roll(dgData["dm07"][0][:, :, np.argmin(np.abs(xc-section))], 0, axis=-1)
        else:
    ...
    for i in range(numRow*numCol):
        ...
        ax[i // numCol, i % numCol].set_xticks(np.arange(0, 110, 1))
        ax[i // numCol, i % numCol].set_xlim(50.5, 52.5)
        ax[i // numCol, i % numCol].set_yticks([0, 2, 4, 6, 8, 10, 12, 14])
        ax[i // numCol, i % numCol].set_ylim(0, 5)
    ...
    #cb.ax.tick_params(labelsize=30)
    #fig.tight_layout()
    plt.savefig(f"./figureS4-1.png", dpi=300)
    plt.clf()
