...
if __name__ == "__main__":
    ...
    for tIdx in timeArange:
        ...
        print("Buoyancy")
        buoyAcce = np.roll(nc.Dataset(config.rebuildDynPath + f"uniform-1/a-{tIdx:06d}.nc")["a"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
        np.mean(np.roll(nc.Dataset(config.rebuildDynPath + f"uniform-1/a-{tIdx:06d}.nc")["a"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax], axis=(1, 2))
        print("DM03")
        dm03Acce = np.roll(nc.Dataset(config.dynTermAccePath + f"uniform-1/a-{tIdx:06d}.nc")["dm04"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
        print("DM0567")
        resAcce =  np.roll(nc.Dataset(config.dynTermAccePath + f"uniform-1/a-{tIdx:06d}.nc")["dm05"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
        resAcce += np.roll(nc.Dataset(config.dynTermAccePath + f"uniform-1/a-{tIdx:06d}.nc")["dm06"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
        resAcce += np.roll(nc.Dataset(config.dynTermAccePath + f"uniform-1/a-{tIdx:06d}.nc")["dm07"][0], -30, axis=-1)[:, yMin:yMax, xMin:xMax]
        resAcce = resAcce / 3
