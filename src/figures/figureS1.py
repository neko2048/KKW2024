...
if __name__ == "__main__":
    ...
    for i, tIdx in enumerate(timeArange):
        ...
        #qc = np.roll(qc, 0, axis=-2)
        #qi = np.roll(qi, 0, axis=-2)
        #qr = np.roll(qr, 0, axis=-2)
        #buoyancy = np.roll(buoyancy, 0, axis=-2)
        #olr = np.roll(olr, 0, axis=-2)
        ...
        plt.plot([27.5, 27.5, 32.5, 32.5, 27.5], [87.5, 92.5, 92.5, 87.5, 87.5], linewidth=2.5, color='black')
        ...
        plt.plot([27.5, 27.5, 32.5, 32.5, 27.5], [87.5, 92.5, 92.5, 87.5, 87.5], linewidth=2.5, color='magenta')
        plt.xlabel("x [km]", fontsize=fs)
        plt.ylabel("y [km]", fontsize=fs)
        plt.xticks(np.arange(0, 101, 10), [], fontsize=fs)
        plt.yticks(np.arange(0, 101, 10), [], fontsize=fs)
        ...
        qcPlot=plt.pcolormesh(xc/1e3, zc/1e3, qc[:, :, np.argmin(np.abs(xc - 90000))]*1e3, cmap=greyCmap, norm=greyNorm)
        plt.contour(xc/1e3, zc/1e3, qi[:, :, np.argmin(np.abs(xc - 90000))]>1e-4, levels=[0.9], colors='green', linewidths=3)
        plt.contourf(xc/1e3, zc/1e3, np.ma.masked_array(np.ones(qr[:, :, np.argmin(np.abs(xc - 90000))].shape), qr[:, :, np.argmin(np.abs(xc - 90000))]<0.5e-4), colors='cyan', alpha=0.25)
        cs = plt.contourf(xc/1e3, zc/1e3, np.ma.masked_array(np.ones(qr[:, :, np.argmin(np.abs(xc - 90000))].shape), qr[:, :, np.argmin(np.abs(xc - 90000))]<10e-4), hatches="/////", colors="None")
        ...
        plt.title(r"(c) Cloud Water / Ice / Rain (y=90km)", fontsize=fs, y=1.015)
        #plt.title("t={hr:02d}h{minute:02d}m".format(
        #        hr = tIdx * config.minPerTimeIdx // 60,
        #        minute = tIdx * config.minPerTimeIdx % 60),
        #        loc='right', fontsize=25, y=1.005)
