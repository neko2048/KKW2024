...
if __name__ == "__main__":
    iniTimeIdx, endTimeIdx = 460, 461 #int(sys.argv[1]), int(sys.argv[2])
    ...
    for tIdx in timeArange:
        ...
        qc = np.roll(thData["qc"][0, :, :, xIdx], 0, axis=-1)
        qv = np.roll(thData["qv"][0, :, :, xIdx], 0, axis=-1)
        qr = np.roll(thData["qr"][0, :, :, xIdx], 0, axis=-1)
        qi = np.roll(thData["qi"][0, :, :, xIdx], 0, axis=-1)
        

        buoyancy = np.roll(nc.Dataset(f"{config.buoyancyPath}buoyancy-{tIdx:06d}.nc")["buoyancy"][0, :, :, xIdx], 0, axis=-1)
        dgData = nc.Dataset(f"{config.dynTermPath}uniform-1/diag-{tIdx:06d}.nc")
        dm03 = np.roll(dgData["dm03"][0, :, :, xIdx], 0, axis=-1)
        dm04 = np.roll(dgData["dm04"][0, :, :, xIdx], 0, axis=-1)
        dm05 = np.roll(dgData["dm05"][0, :, :, xIdx], 0, axis=-1)
        dm06 = np.roll(dgData["dm06"][0, :, :, xIdx], 0, axis=-1)
        dm07 = np.roll(dgData["dm07"][0, :, :, xIdx], 0, axis=-1)
        ...
        buoyancy = np.roll(nc.Dataset(f"{config.rebuildDynPath}uniform-1/a-{tIdx:06d}.nc")["a"][0, :, :, xIdx], 0, axis=-1)
        ...
        plt.savefig(f"figureS2.png", dpi=300)
